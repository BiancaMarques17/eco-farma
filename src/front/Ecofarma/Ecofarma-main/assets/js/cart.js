...
async function mostrarModalPontos() {
  ...
  try {
    const response = await fetch(`http://ecofarma-f4ake0gkhwapfmh3.canadacentral-01.azurewebsites.net/api/cupom/cliente/${id}`);
...
async function finalizarCompra() {
  ...
  if (pontosUsados > 0) {
    try {
      const r = await fetch(`http://ecofarma-f4ake0gkhwapfmh3.canadacentral-01.azurewebsites.net/api/cupom/cliente/${id}`);
      const cupons = await r.json();

      for (let i = 0; i < pontosUsados && i < cupons.length; i++) {
        await fetch(`http://ecofarma-f4ake0gkhwapfmh3.canadacentral-01.azurewebsites.net/api/cupom/${cupons[i].id_cupom}`, {
          method: "DELETE"
        });
      }
    } catch (err) {
